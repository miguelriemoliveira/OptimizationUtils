...
if __name__ == "__main__":
    ...
    detected_collections = 0

    # points_model = np.zeros((2, n_points*detected_collections), np.float32)
    # points = np.zeros((2, n_points*detected_collections), np.float32)
    ...
    for collection_key, collection in data['collections'].items():
        if not (collection['labels'][sensor_2]['detected'] and collection['labels'][sensor_1]['detected']) :
            ...
            root_T_chessboard = utilities.translationQuaternionToTransform(data['chessboards']['collections'][collection_key]['trans'],
    ...
    print(int(48*detected_collections))

    # print '\nSize of the points and points model:\n '
    # print(points.shape)
    # # print(points)
    # print(points_model.shape)
    ...
    plt.scatter(points_model[0, :], points_model[1, :], label='points obtained with openCV homography finder', color='b')
    plt.legend()
